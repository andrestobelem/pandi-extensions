...
## When to use it
- "Mejorá / improve this package", "do an improvement pass", "harden the extensions or e2e", "iterate until dry".
...
- `allow` — editable globs (default `extensions/loop.ts`, `extensions/goal.ts`, `extensions/plan.ts`, `examples/**`, `docs/**`). Narrow it (e.g. `["docs/**"]`) for the lowest-risk first test.
- `hotFiles` — never-edit paths (default `extensions/dynamic-workflows.ts`).
- `verifyCmd` (default `npm test`) — the objective green/red check that gates each pass.
- `logPath` (default `docs/investigaciones/continuous-improvement-log.md`) — the progress log it appends to.
...
4. **Verify:** the workflow itself runs `verifyCmd`; a RED tree forces the pass to `BLOCKED` (it never continues on red). It appends a log entry.
...
- Does NOTHING irreversible: no push, no `rm -rf`, no deleting/committing others' work.

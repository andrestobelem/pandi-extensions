/**
 * Integration tests for the read-only safety gate in extensions/pandi-plan/index.ts.
 *
 ...
 *
 * Run it:
 *   node extensions/pandi-plan/tests/integration/plan-gate.test.mjs
 */
...
async function planGate(planUrl) {
	...
	// ALLOWED: read-only bash + read tools + submit_plan. The last five are read-only commands
	// whose quoted/comparison operators (>, ->, >=, =>) must NOT be mistaken for write redirections.
	for (const cmd of [
		"git ls-files",
		"cat package.json",
		"grep -n foo bar.ts",

...
function makeLoopIterationPrompt(loop: LoopState): string {
	const lines: string[] = [];
...
function fireWake(pi: ExtensionAPI, ctx: ExtensionContext, loop: ActiveLoop): void {
	...
	if (Date.now() - loop.startedAt >= WATCHDOG_HARD_DEADLINE_MS) {
		const reason = `watchdog: exceeded hard backstop deadline (${Math.round(WATCHDOG_HARD_DEADLINE_MS / 3600000)}h)`;
		stopLoop(pi, ctx, loop.loopId, reason, "done");
...
function rearmFixed(pi: ExtensionAPI, ctx: ExtensionContext, loop: ActiveLoop): void {
	...
	loop.lastReason = `auto: fixed interval ${formatInterval(Math.round(period / 1000))}`;
	loop.rearmedThisTurn = true;
	persist(pi, ctx, loop);
	setLoopStatus(ctx, loop);
...
function resumeLoop(pi: ExtensionAPI, ctx: ExtensionContext, loop: ActiveLoop): boolean {
	...
	scheduleWake(pi, ctx, loop, Math.round(remaining / 1000), "resumed by user");
	return true;
}
...
async function rehydrate(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
	...
	// Resolve each loopId against its sidecar (newer-by-updatedAt wins).
...
function isUnsafeWritePath(ctx: ExtensionContext, filePath: unknown): boolean {
...
export default function loopExtension(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		label: "Loop Schedule",
		description:
			"Schedule the next iteration of the active /loop. Call this when more work or waiting is needed before the next pass.",
	...
	pi.on("session_shutdown", async (_event, ctx) => {
		...
		// Clear the wake queue and in-flight gate (no stale wakes survive a shutdown;
		// rehydrate rebuilds the live set on next start).
		wakeQueue.length = 0;
		autopilotTurnInFlight = false;
		clearLoopStatus(ctx);
	});

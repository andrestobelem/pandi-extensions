...
- **Path/layout helpers en lib/paths.ts:** constantes `WORKFLOW_*`, `slugify`, `projectHash`, `ensureDir`, roots de run/graph y `createRunDirectory` viven en `lib/paths.ts` y se reexportan desde `lib/index.ts` y `surface/index.js`. `runtime` importa paths desde lib; la dependencia runtime→surface restante es `resolveWorkflow` / `preflightWorkflowLaunch` (y subworkflow resolve en `runtime/subworkflow.ts`).

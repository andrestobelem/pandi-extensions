...
function withMissingSkill(skillName, fn) {
...
function main() {
	...
	withMissingSkill("github-project", () => {
		const mirrorCheck = runNode(MIRRORS, ["--check"]);
		check(
	...
	withMissingSkill("deep-research", () => {
		const vendorCheck = runNode(VENDOR, ["--check"]);
		check(
	...
	const unclassifiedDir = fs.mkdtempSync(path.join(SKILLS_ROOT, "__unclassified-skill-"));
	...
	try {
		...
		const claudeSync = runNode(CLAUDE, ["--dest", tmpClaude]);
		check(
		...
		const claudeCheck = runNode(CLAUDE, ["--dest", tmpClaude, "--check"]);
		check(

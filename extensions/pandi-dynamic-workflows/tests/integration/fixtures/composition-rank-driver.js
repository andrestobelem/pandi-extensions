export default async function main() {
	...
	const generatorText = await agent(
	...
	let candidates = [];
	try {
		candidates = JSON.parse(generatorText);
	} catch {
		candidates = [];
	}
	...
	if (candidates.length >= maxCandidates)
		await log("candidate cap applied", { generated: candidates.length, maxCandidates });
	...
	const ranking = await workflow("lib/rank-candidates", {
		candidates,
		goal,
		rubric: input?.rubric,
	...
	const synthesisText = await agent(
		`Explain why the top-ranked candidate won and how it compares to the runners-up. ` +
			...
			`${compact(ranking, 50000)}`,
		{ name: "rank-synthesis", agentType: "reviewer", tools: ["read", "grep", "find", "ls"] },
	);
	await writeArtifact("best.md", synthesisText);

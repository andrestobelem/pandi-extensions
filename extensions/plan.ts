 ...
 *
 * AUTONOMOUS: this file does not import from extensions/loop.ts or extensions/goal.ts;
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if (approved) {
				...
				notify(ctx, `Plan ${plan.planId} approved. Exiting plan mode and implementing.`, "info");
				return {
					content: [{ type: "text" as const, text: "Plan approved — implementing now." }],
					details: { planId: plan.planId, status: "approved" },
				};
			}
			...
			plan.rejections += 1;
			plan.status = "planning"; // remains active; status reflects we're still planning.
			persist(pi, plan);
			setPlanStatus(ctx, plan);
			notify(ctx, `Plan ${plan.planId} rejected. Still in plan mode; the agent will revise.`, "info");
			return {
				content: [
					{
						type: "text" as const,
				...
				details: { planId: plan.planId, status: "rejected", rejections: plan.rejections },
			};
		},
	});

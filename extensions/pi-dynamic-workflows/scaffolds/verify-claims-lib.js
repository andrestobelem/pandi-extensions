...
module.exports = async function workflow(ctx, input) {
  ...
  for (let i = 0; i < claims.length; i++) {
    ...
    const record = { claim, parsedVotes: cast, failedBranches: jury.filter((r) => !(r && r.data)).length, refutations, survived };
    votes.push(record);
    if (survived) verified.push({ ...claim, verification: record });
    else dropped.push({ ...claim, verification: record });
    await ctx.log("claim verification complete", { index: i + 1, total: claims.length, survived, refutations, votes: cast.length, failedBranches: record.failedBranches });
  }

...
module.exports = async function workflow(ctx, input) {
	...
	const reviewed = await ctx.pipeline(
		...
		(file, _orig, i) =>
			ctx
				.agent(`Classify the risk of bugs/security issues in ${file}. Be quick; do not deep-dive.`, {
					name: `classify-${i}`,
					agentType: "reviewer",
					tools: ["read", "grep"],
					schema: VERDICT,
	...
	const findings = reviewed.filter(Boolean).filter((c) => c.deep && !/NO_FINDINGS/.test(c.deep));

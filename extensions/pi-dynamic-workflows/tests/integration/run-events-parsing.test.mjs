...
async function buildExtension() {
	const outDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-run-events-integration-"));
...
function scenarioValueCoercers(mod) {
	...
	check("numberValue keeps a finite number incl. 0", numberValue(5) === 5 && numberValue(0) === 0, `${numberValue(5)}/${numberValue(0)}`);
	check("numberValue rejects NaN/Infinity/strings", numberValue(NaN) === undefined && numberValue(Infinity) === undefined && numberValue("5") === undefined, "NaN/Inf/'5'");
	check("booleanValue keeps booleans", booleanValue(true) === true && booleanValue(false) === false, `${booleanValue(true)}/${booleanValue(false)}`);
	check("booleanValue rejects truthy non-booleans", booleanValue(0) === undefined && booleanValue("true") === undefined, "0/'true'");

	// stringArrayValue: array of ALL strings -> array; any non-string -> undefined; [] -> [].
	check("stringArrayValue keeps an all-string array", JSON.stringify(stringArrayValue(["a", "b"])) === JSON.stringify(["a", "b"]), JSON.stringify(stringArrayValue(["a", "b"])));
	check("stringArrayValue rejects a mixed array", stringArrayValue(["a", 1]) === undefined, String(stringArrayValue(["a", 1])));
	check("stringArrayValue keeps an empty array", Array.isArray(stringArrayValue([])) && stringArrayValue([]).length === 0, JSON.stringify(stringArrayValue([])));
	...
	check("isAgentMonitorState rejects others", isAgentMonitorState("nope") === false && isAgentMonitorState(undefined) === false, "nope/undefined");
}
...
function scenarioDerivations(mod) {
	...
	check("getAgentElapsedMs returns the recorded elapsedMs", getAgentElapsedMs({ state: "running", elapsedMs: 1234 }) === 1234, String(getAgentElapsedMs({ state: "running", elapsedMs: 1234 })));
	check("getAgentElapsedMs is undefined for a finished agent with no elapsedMs", getAgentElapsedMs({ state: "completed" }) === undefined, String(getAgentElapsedMs({ state: "completed" })));
	const future = new Date(Date.now() + 1_000_000).toISOString();
	check("getAgentElapsedMs clamps a future startedAt to 0 while running", getAgentElapsedMs({ state: "running", startedAt: future }) === 0, String(getAgentElapsedMs({ state: "running", startedAt: future })));
	check("getAgentElapsedMs is undefined when running with no startedAt", getAgentElapsedMs({ state: "running" }) === undefined, String(getAgentElapsedMs({ state: "running" })));
	check("getAgentElapsedMs is undefined for an unparseable startedAt", getAgentElapsedMs({ state: "running", startedAt: "nope" }) === undefined, String(getAgentElapsedMs({ state: "running", startedAt: "nope" })));

	// formatAgentPhase: needs truthy index AND total; phaseId prefixes "P{id} ".
	check("formatAgentPhase renders index/total", formatAgentPhase({ phaseIndex: 1, phaseTotal: 3 }) === "1/3", String(formatAgentPhase({ phaseIndex: 1, phaseTotal: 3 })));
	check("formatAgentPhase prefixes the batch id", formatAgentPhase({ phaseId: 2, phaseIndex: 1, phaseTotal: 3 }) === "P2 1/3", String(formatAgentPhase({ phaseId: 2, phaseIndex: 1, phaseTotal: 3 })));
	check("formatAgentPhase is undefined when index is 0/falsy", formatAgentPhase({ phaseIndex: 0, phaseTotal: 3 }) === undefined, String(formatAgentPhase({ phaseIndex: 0, phaseTotal: 3 })));
	check("formatAgentPhase is undefined without a total", formatAgentPhase({ phaseIndex: 1 }) === undefined, String(formatAgentPhase({ phaseIndex: 1 })));

	// phaseEventFields: empty for missing/zero-total phase; otherwise id/index/total (+ optional label).
	check("phaseEventFields is empty for undefined", JSON.stringify(phaseEventFields(undefined)) === "{}", JSON.stringify(phaseEventFields(undefined)));
	check("phaseEventFields is empty for total <= 0", JSON.stringify(phaseEventFields({ id: 1, index: 1, total: 0 })) === "{}", JSON.stringify(phaseEventFields({ id: 1, index: 1, total: 0 })));
	check(
		"phaseEventFields maps id/index/total without a label",
		JSON.stringify(phaseEventFields({ id: 2, index: 1, total: 3 })) === JSON.stringify({ phaseId: 2, phaseIndex: 1, phaseTotal: 3 }),
...
function scenarioMergeAgentMonitor(mod) {
	...
	const fresh = mergeAgentMonitor(undefined, { id: 1, name: "alpha", state: "running", startedAt: "T1" });
	check("merge: new agent keeps id/name/state/startedAt", fresh.id === 1 && fresh.name === "alpha" && fresh.state === "running" && fresh.startedAt === "T1", JSON.stringify(fresh));
	check("merge: new agent without artifact is promptAvailable=false", fresh.promptAvailable === false, JSON.stringify(fresh));
	check("merge: new running agent has no endedAt", fresh.endedAt === undefined, JSON.stringify(fresh));

	const finishedToRunning = mergeAgentMonitor({ id: 1, name: "alpha", state: "completed" }, { id: 1, name: "alpha", state: "running" });
	check("merge: a completed agent is NOT regressed to running", finishedToRunning.state === "completed", finishedToRunning.state);
	const cachedToRunning = mergeAgentMonitor({ id: 1, name: "alpha", state: "cached" }, { id: 1, name: "alpha", state: "running" });
	check("merge: a cached agent is NOT regressed to running", cachedToRunning.state === "cached", cachedToRunning.state);

	const ended = mergeAgentMonitor({ id: 1, name: "alpha", state: "running", startedAt: "T1" }, { id: 1, name: "alpha", state: "completed", endedAt: "T2", elapsedMs: 7 });
	check("merge: running -> completed applies the terminal state", ended.state === "completed" && ended.endedAt === "T2" && ended.elapsedMs === 7, JSON.stringify(ended));
	check("merge: running -> completed preserves the earlier startedAt", ended.startedAt === "T1", JSON.stringify(ended));
	...
	const withArtifact = mergeAgentMonitor(undefined, { id: 1, name: "alpha", artifactPath: "/runs/agents/0001-alpha.md" });
	check("merge: an artifactPath makes promptAvailable=true", withArtifact.promptAvailable === true && withArtifact.artifactPath === "/runs/agents/0001-alpha.md", JSON.stringify(withArtifact));
	const keepsPrompt = mergeAgentMonitor({ id: 1, name: "alpha", state: "running", promptAvailable: true }, { id: 1, name: "alpha" });
	check("merge: an existing promptAvailable=true is preserved", keepsPrompt.promptAvailable === true, JSON.stringify(keepsPrompt));
}
...
async function scenarioReadRunEvents(mod) {
	...
	const lines = [
		...
		JSON.stringify({ type: "log", time: "T2", message: "agent 1 end: my agent", details: { ok: true, code: 0, elapsedMs: 500 } }),
	...
	check("readRunEvents collects only the log events (malformed/blank lines ignored)", parsed.logs.length === 3, `logs=${parsed.logs.length}`);
	check("readRunEvents preserves log order", parsed.logs.map((l) => l.time).join(",") === "T1,T2,T3", parsed.logs.map((l) => l.time).join(","));
	check("readRunEvents keeps the first log message verbatim", parsed.logs[0].message === "agent 1 start: my agent", parsed.logs[0].message);

	check("readRunEvents derives 2 agents sorted by id", parsed.agents.length === 2 && parsed.agents[0].id === 1 && parsed.agents[1].id === 2, JSON.stringify(parsed.agents.map((a) => a.id)));
	const a1 = parsed.agents.find((a) => a.id === 1);
	check(
		"readRunEvents merges 'agent N start' + 'agent N end' (ok:true) into a completed agent",
		a1.state === "completed" && a1.name === "my agent" && a1.startedAt === "T1" && a1.endedAt === "T2" && a1.elapsedMs === 500 && a1.code === 0 && a1.ok === true,
		JSON.stringify(a1),
	);
	const a2 = parsed.agents.find((a) => a.id === 2);
	check("readRunEvents maps an 'agent' event with ok:false to a failed agent", a2.state === "failed" && a2.name === "second" && a2.code === 1 && a2.ok === false, JSON.stringify(a2));

	const emptyDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-run-events-empty-"));
	const missing = await readRunEvents(emptyDir);
	check("readRunEvents tolerates a missing events.jsonl", missing.logs.length === 0 && missing.agents.length === 0, JSON.stringify(missing));
}

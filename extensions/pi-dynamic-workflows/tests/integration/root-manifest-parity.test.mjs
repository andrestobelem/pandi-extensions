 ...
 * and `pi.themes` are DERIVED from each `extensions/pi-<name>/package.json` pi manifest by
 ...
 *   - Coverage: every extensions/pi-<name> package that declares pi.extensions appears in the
 ...
 *
 * Run it:
 *   node extensions/pi-dynamic-workflows/tests/integration/root-manifest-parity.test.mjs
 */
...
function main() {
	...
	const dirs = fs
		...
		.filter((d) => d.startsWith("pi-") && fs.existsSync(path.join(extensionsDir, d, "package.json")));

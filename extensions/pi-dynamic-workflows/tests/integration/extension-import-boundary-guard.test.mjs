 ...
 * (the files directly at each extension's root dir under `extensions/`, e.g. `extensions/pi-goal/` —
 ...
 * file (extensions/pi-goal/index.ts), asserts the scanner flags exactly that escape, then
 ...
 *
 * Run it:
 *   node extensions/pi-dynamic-workflows/tests/integration/extension-import-boundary-guard.test.mjs
 */
...
async function main() {
	...
	check("found extension directories to scan under extensions/pi-*", extDirs.length > 0, `count=${extDirs.length}`);
	...
	const goalDir = path.join(EXTENSIONS_DIR, "pi-goal");
	...
	await withMutatedFile(
		...
		() => {
			...
			check(
				"escaping ../shared/runtime.js import injected into pi-goal/index.ts is flagged",
	...
	check(
		"pi-goal/index.ts is restored to zero escapes after the negative control",
	...
	await withMutatedFile(
		...
		() => {
			const mutatedEscapes = scanExtension(goalDir).filter((r) => r.kind === "escape");
			check(
				'escaping dynamic import("../shared/lazy.js") injected into pi-goal/index.ts is flagged',
	...
	check(
		"pi-goal/index.ts is restored to zero escapes after the dynamic-import control",
		scanExtension(goalDir).filter((r) => r.kind === "escape").length === 0,
	);

...
import { buildClaudeCommand, invokeClaude, resolveClaudeModel } from "./claude-agent.mjs";
...
async function resolveWorkflow(cwd, name) {
	...
	const candidates = [
		{ root: path.join(cwd, ".claude", "ultracode", "workflows"), scope: "claude-project" },
		{ root: packageWorkflowsDir(), scope: "claude-host" },
...
function makeApi({ state, depth, sourcePath }) {
	...
	const agent = async (prompt, rawOptions = {}) => {
		...
		if (unsupported) throw new Error(`Claude host does not support per-agent ${unsupported}; refusing to ignore it.`);
		if (rawOptions.allowWrite) {
			throw new Error("Claude host is read-only; agent write access is not supported.");
		}
		...
		const model = resolveClaudeModel(effective.model ?? options.model, options.env);
		if (model.warning) await log(model.warning);
		const effort = effective.effort ?? effective.thinking ?? options.effort;
		if (effort) await log(`Claude effort ${JSON.stringify(effort)} forwarded to Claude CLI.`);
		const command = buildClaudeCommand({
			command: options.claudeCommand,
			commandArgs: options.claudeCommandArgs,
			cwd,
			model: model.model,
			effort,
		...
		for (let attempt = 0; attempt <= retries; attempt++) {
			...
			final = { ok: false, error: `Claude response failed schema validation: ${JSON.stringify(checked.errors)}` };
		...
		if (!final?.ok) {
			...
			if (effective.schema && effective.schemaOnInvalid !== "null")
				throw new Error(final?.error ?? "Claude agent failed.");
			return null;
		}
	...
	const ask = async (question, askOptions = {}) => {
		if (!process.stdin.isTTY || !process.stdout.isTTY) {
			...
			throw new Error(`ask() needs a TTY host; pass a default for headless Claude runs. Question: ${question}`);
		}
	...
	return {
		...
		ask,
		bash: async () => {
			throw new Error("bash() is not supported by the read-only Claude host.");
		},
		readFile: async (file, encoding = "utf8") => await fs.readFile(inside(cwd, file), encoding),
		writeFile: async () => {
			throw new Error("writeFile() is not supported by the read-only Claude host.");
		},
		appendFile: async () => {
			throw new Error("appendFile() is not supported by the read-only Claude host.");
		},

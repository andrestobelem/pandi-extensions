...
Run flags:
  ...
  --concurrency <n>           Maximum Claude workers in flight (default: 4)
  --max-agents <n>            Maximum launched Claude workers (default: 32)
...
function common(flags) {
	return {
		...
		model: flags.model,
		effort: flags.effort,
		claudeCommand: flags["claude-command"],
...
async function main() {
	...
	if (action === "doctor") {
		const command = flags["claude-command"] ?? process.env.PANDI_CLAUDE_COMMAND ?? "claude";
		const probe = spawnSync(command, ["--version"], { encoding: "utf8" });
		if (probe.error || probe.status !== 0)
			throw new Error(`Could not run ${command} --version: ${probe.error?.message ?? probe.stderr.trim()}`);
		console.log(`Claude CLI: ${(probe.stdout || probe.stderr).trim()}`);

...
export function transformWorkflowCode(source) {
	if (/^\s*import\s/m.test(source)) {
		throw new Error(
			"Static import statements are not supported in portable host workflows. Use injected workflow globals instead.",
		);
	}
	...
	if (/^\s*export\s/m.test(output)) {
		throw new Error("Only `export const meta` and `export default` are supported in portable host workflows.");
	}

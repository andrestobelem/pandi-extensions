...
import { buildCodexCommand, invokeCodex, resolveCodexModel } from "./codex-agent.mjs";
...
async function resolveWorkflow(cwd, name) {
	...
	const candidates = [
		{ root: path.join(cwd, ".codex", "ultracode", "workflows"), scope: "codex-project" },
		{ root: packageWorkflowsDir(), scope: "codex-host" },
...
function makeApi({ state, depth, sourcePath }) {
	...
	const agent = async (prompt, rawOptions = {}) => {
		...
		if (unsupported) throw new Error(`Codex host does not support per-agent ${unsupported}; refusing to ignore it.`);
		if (rawOptions.allowWrite) {
			throw new Error("Codex host is read-only; agent write access is not supported.");
		}
		...
		const model = resolveCodexModel(effective.model ?? options.model, options.env);
		if (model.warning) await log(model.warning);
		const effort = effective.effort ?? effective.thinking ?? options.effort;
		if (effort)
			await log(`Codex effort ${JSON.stringify(effort)} is advisory; this host does not infer config overrides.`);
		const lastMessageFile = `${stem}.final.md`;
		const outputSchemaFile = effective.schema ? `${stem}.schema.json` : undefined;
		if (outputSchemaFile) await writeJson(outputSchemaFile, effective.schema);
		...
		for (let attempt = 0; attempt <= retries; attempt++) {
			...
			final = { ok: false, error: `Codex response failed schema validation: ${JSON.stringify(checked.errors)}` };
		...
		if (!final?.ok) {
			...
			if (effective.schema && effective.schemaOnInvalid !== "null")
				throw new Error(final?.error ?? "Codex agent failed.");
			return null;
		}
	...
	const ask = async (question, askOptions = {}) => {
		if (!process.stdin.isTTY || !process.stdout.isTTY) {
			...
			throw new Error(`ask() needs a TTY host; pass a default for headless Codex runs. Question: ${question}`);
		}
	...
	return {
		...
		ask,
		bash: async () => {
			throw new Error("bash() is not supported by the read-only Codex host.");
		},
		readFile: async (file, encoding = "utf8") => await fs.readFile(inside(cwd, file), encoding),
		writeFile: async () => {
			throw new Error("writeFile() is not supported by the read-only Codex host.");
		},
		appendFile: async () => {
			throw new Error("appendFile() is not supported by the read-only Codex host.");
		},
		listFiles: async (directory = ".") => await listFilesRecursive(inside(cwd, directory)),
	};
}

/** Run a portable workflow locally while Codex CLI supplies each agent() worker. */
export async function runWorkflow(rawOptions) {
	...
	const options = {
		...
		codexCommand: rawOptions.codexCommand ?? process.env.PANDI_CODEX_COMMAND ?? "codex",
		codexCommandArgs: rawOptions.codexCommandArgs ?? [],
	...
	if (!options.trustWorkspace) {
		throw new Error("Codex runs non-interactively; pass --trust-workspace to acknowledge this workspace.");
	}
	...
	if (!runDir.startsWith(path.join(cwd, ".codex", "ultracode", "runs") + path.sep)) {
		throw new Error("Run directory must stay below .codex/ultracode/runs.");
	}

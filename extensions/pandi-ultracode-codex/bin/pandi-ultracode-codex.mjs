...
Run flags:
  ...
  --concurrency <n>           Maximum Codex workers in flight (default: 4)
  --max-agents <n>            Maximum launched Codex workers (default: 32)
...
async function main() {
	...
	if (action === "doctor") {
		const command = flags["codex-command"] ?? process.env.PANDI_CODEX_COMMAND ?? "codex";
		const probe = spawnSync(command, ["--version"], { encoding: "utf8" });
		if (probe.error || probe.status !== 0)
			throw new Error(`Could not run ${command} --version: ${probe.error?.message ?? probe.stderr.trim()}`);
		console.log(`Codex CLI: ${(probe.stdout || probe.stderr).trim()}`);
		console.log("Output protocol: Codex exec JSONL (verified by the bundled adapter)");
		return;
	}

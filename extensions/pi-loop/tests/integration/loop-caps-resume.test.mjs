/**
 * Durable behavioral integration test for the DURABILITY surface of extensions/pi-loop/index.ts:
 * the CAPS gate, PAUSE/RESUME, and loop REHYDRATE — none of which is covered by
 * loop-behavior.test.mjs (that suite covers FIFO serialization, the fixed-mode
 ...
 *
 * What it covers (all DISTINCT from loop-behavior.test.mjs — no duplication):
 ...
 *      session_start), keeps paused as paused, ignores terminal, survives same-process
 *      shutdown→startup, last-wins by updatedAt across the JSONL and sidecar-only crash
 *      state, retires an autonomous loop when the project is no longer trusted, and a cap
 *      already blown across downtime stops cleanly.
 *
 * How it works
 * ------------
 * Self-bootstrapping, same proven pattern as loop-behavior / safety-gates / goal-* integration tests:
 * esbuild the CURRENT extensions/pi-loop/index.ts into an OS temp dir at run time (never a stale
 ...
 *
 * Run it:
 *   node extensions/pi-loop/tests/integration/loop-caps-resume.test.mjs
...
// ===========================================================================
// SCENARIO C5: same-process shutdown→startup. session_shutdown persists stale
// state, clears timers, and may be followed by a fresh session_start in the same
// extension process. The in-memory stale loop must NOT make rehydrate skip the
...
// ===========================================================================
// SCENARIO C6: sidecar-only recovery. If a crash leaves state.json present but
// the session JSONL lacks that loopId, rehydrate must still recover it even when

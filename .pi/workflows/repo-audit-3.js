...
export default async function main() {
	...
	const items = [
		...
		item("pandi-goal", "sonnetHigh", "extensions/pandi-goal/*.ts (skip tests/). Focus on activeGoals cleanup on stop/shutdown (delete/clear symmetry vs pi-loop), sidecar write-vs-read symmetry, independent-verifier gating & caps, use-after-shutdown of in-flight verifiers, and iteration/wait clamps."),
		item("pandi-goal", "codexHigh", "extensions/pandi-goal/*.ts (skip tests/). Focus on activeGoals cleanup on stop/shutdown (delete/clear symmetry vs pi-loop), sidecar write-vs-read symmetry, independent-verifier gating & caps, use-after-shutdown of in-flight verifiers, and iteration/wait clamps."),

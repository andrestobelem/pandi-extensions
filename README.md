...
| Docs de librerías on-demand (Context7) | skill `context7-cli` (global) + CLI `ctx7` | Instalá la skill una vez con `ctx7 skills install context7-cli` (queda en `~/.agents/skills/`). El CLI `ctx7` es devDependency: corre con `npx ctx7` tras `npm install` (o global: `npm i -g ctx7@latest`) |
...
- **Context7 (docs de librerías)** — el skill `context7-cli` **no** viene vendorizado en el repo: instalalo **globalmente** una sola vez con `ctx7 skills install context7-cli` (queda en `~/.agents/skills/context7-cli/`, o copialo a `~/.pi/agent/skills/`). Pi lo autodescubre desde el scope global en cualquier proyecto y lo agrega a los subagentes. El CLI `ctx7` viene como **devDependency**: corre con `npx ctx7` tras `npm install` (o global con `npm i -g ctx7@latest`). Opt-out por subagente: `includeSkills: false`.

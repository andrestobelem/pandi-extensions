...
- **Context7 (docs de librerías)** — el skill `context7-cli` ya viene **vendorizado** en este repo (`.agents/skills/context7-cli/`, junto a los demás skills de proyecto como `karpathy-guidelines`), así que Pi lo **autodescubre** al trabajar dentro de este repo y lo agrega a los subagentes sin instalar nada aparte. Es una skill de proyecto (se descubre por cwd, no se propaga con `pi install ./`): para tenerla fuera del repo, copiala a `~/.pi/agent/skills/` o instalala con `ctx7 skills install ...`. Solo necesitás el CLI `ctx7`, que viene como **devDependency**: corre con `npx ctx7` tras `npm install` (o global con `npm i -g ctx7@latest`). Opt-out por subagente: `includeSkills: false`.

...
Esta página es el mapa del catálogo de scaffolds de `pandi-extensions`. Un scaffold es una plantilla ejecutable lista para instanciar (agentes, paralelismo, loops, votación, etc.) que resuelve una forma recurrente de tarea. Si ya sabés qué forma tiene tu problema, andá directo a la categoría; si no, empezá por la tabla.
...
- [`large-migration`](./large-migration.md): Aplicador real: gate de baseline en verde; por archivo aplica → verifica → repara acotado, con rollback ante fallo. Secuencial.

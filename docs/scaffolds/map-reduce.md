...
**Caché y resume:** en una primera corrida, cada llamada a `agent` se lanza normalmente. El runtime journaliza las llamadas completadas y, al reanudar el mismo run, reutiliza por defecto su resultado en caché en lugar de volver a invocarlas. Para forzar una ejecución nueva en una llamada, pasá `{ cache: false }` en sus opciones.

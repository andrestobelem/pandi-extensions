...
- **pi-auto-compact-context (120):** opcional `threshold.ts` (`index.ts:3-11`, ~12 LOC) **re-exportando** `parseThreshold`/`DEFAULT_THRESHOLD_PERCENT`. Mayor valor: extraer decisión pura `decideCompaction` para tests rápidos, sin file-split obligatorio.
...
Estado verificado: **no existe** harness compartido; cada una de las 24 suites reimplementa el bootstrap esbuild→tempdir→`pathToFileURL`, con variantes incompatibles (`npx --no-install esbuild` en `auto-compact-context.test.mjs:39`/`local-memory.test.mjs:39`; `npx esbuild` plano en effort/mdview/bg/plan; `npx --yes esbuild` con 2 alias en `goal-verifier.test.mjs:94-100`; con 5 alias en `composition-rank.test.mjs:79-87`).
...
| 2.3 | `refactor(shared)` | `extensions/shared/state-store.ts`, `pi-loop`, `pi-goal` | `writeSidecarAtomic<T>` + `stateDir` dual-root (near-identical) | loop-caps-resume + goal-rehydrate + `npm test` | Medium |
...
| 3.2 | `refactor(auto-compact)` | `pi-auto-compact-context/threshold.ts` + `index.ts` | `parseThreshold`/`DEFAULT_THRESHOLD_PERCENT` (re-export para test `:122`) | auto-compact-context.test.mjs + `npm test` | Low |

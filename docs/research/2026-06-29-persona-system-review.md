...
Revisar el subsistema de **Personas** de `pi-dynamic-workflows`: dónde se definen, cómo se
resuelven/fusionan, y qué riesgos de correctitud / seguridad / consistencia tiene. Lente:
skeptical code reviewer, read-only, con cita `file:line`.
...
- **Path traversal bloqueado**: el regex `/^[a-zA-Z0-9._-]+$/` (`:223`) prohíbe `/` y `\`, así
  que `${name}.json` no escapa del dir de personas.
- `sanitizePersonaOptions` (`:199`) **no** copia `agentType` → un JSON de proyecto no puede
  re-disparar la resolución (sin recursión) ni inyectar `schema`/`prompt`.
- `appendSystemPrompt` se concatena de forma determinista (`:211`); merge determinista; los
  niveles de `thinking` respetan el contrato documentado.

## Hallazgos (por peso)

### 1. `systemPrompt` se pisa, `appendSystemPrompt` se fusiona — asimetría
`mergePersonaOptions` (`agent-env-persona.ts:210‑219`) hace `{...persona, ...options}`. La
**identidad** de cada persona (lo que hace que `reviewer` sea escéptico) vive en `systemPrompt`,
que es un campo pisable: si el caller pasa su propio `systemPrompt`, **borra entero** el de la
persona y sólo `appendSystemPrompt` sobrevive (concatenado). Resultado: podés pedir
`agentType:"reviewer"` y, si además pasás `systemPrompt`, la reviewer-ness desaparece en
silencio. Es coherente con "lo explícito gana", pero el comportamiento identitario quedó en el
campo clobbereable.
...
Built-in: `BUILTIN_AGENT_PERSONAS[name.toLowerCase()]` (`:246`). Proyecto: `${name}.json` con
case preservado (`:230‑231`). En FS case-sensitive (Linux/CI), `agentType:"Reviewer"` **no**
encuentra `Reviewer.json` pero **sí** el built-in `reviewer` → usa el built-in en silencio aunque
hubiera un override de proyecto. En macOS el FS lo tapa.
...
`Array.isArray(out.tools)` (`:329`), otros consumidores asumen array.
...
**también** override `tools` (con `approve` solo no alcanza). El comportamiento es seguro/
conservador; sólo la redacción induce a error.
...
`scaffolds/` lo usa; ahí se rutea todo por `node(role)` + mapas `models{}`/`efforts{}`. Conviven
dos mecanismos: **persona** (`systemPrompt` + `thinking`) vs **node-role** (`model`/`effort`). No
es bug, pero los scaffolds hoy sólo heredan model/effort, no la **identidad** (el systemPrompt
...
lo hacen el README raíz (`:158,402,500,525`) y el skill.

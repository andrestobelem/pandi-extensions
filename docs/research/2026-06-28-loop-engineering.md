...
1. **Terminación acotada (sin loops infinitos).** Todo loop necesita una condición de parada. Reflexion acota la memoria; Self-Refine acota iteraciones ([2303.11366](https://arxiv.org/abs/2303.11366), [2303.17651](https://arxiv.org/abs/2303.17651)). Repo: pi-goal declara "Never an infinite loop" (`extensions/pandi-goal/index.ts:51`).
...
| Cap de iteración (gate separado) | `extensions/pandi-loop/index.ts:416-419`, `:486-490`; `maxIterations=25` por defecto (docstring `:72`); pi-goal default 30 (`extensions/pandi-goal/constants.ts:32-40`) | **Aligned** — el cap es un gate distinto de `capExceeded`, como dice la read-list. |

...
Esta guía registra la configuración activa de [kitty](https://sw.kovidgoyal.net/kitty/) que usamos para trabajar en este
repo: correr Pi, Supacode y los dynamic workflows.

Se versiona **solo lo que está activo**. El `kitty.conf` real generado por kitty trae ~3000 líneas de defaults
comentados; acá guardamos únicamente los ajustes que cambiamos, para que sea legible y reproducible. Todo lo demás usa
los defaults de kitty.

- **Fecha de captura:** 2026-07-03
- **kitty:** 0.47.4
- **Archivos:** [`config/kitty/kitty.conf`](../config/kitty/kitty.conf),
  [`config/kitty/current-theme.conf`](../config/kitty/current-theme.conf) y los temas automáticos día/noche
  `light-theme.auto.conf` / `dark-theme.auto.conf` / `no-preference-theme.auto.conf`

## En 30 segundos

Si querés reproducir este entorno, instalá los requisitos, copiá los archivos versionados a `~/.config/kitty/` y recargá
kitty. Eso te deja el mismo tema (día/noche automático), fuente, scrollback, tabs, tamaño de ventana y control remoto
que usamos en desarrollo.

## Requisitos

- **kitty** ≥ 0.47 (`brew install --cask kitty` en macOS).
- **FiraCode Nerd Font Mono** instalada (`brew install --cask font-fira-code-nerd-font`). Es una Nerd Font: incluye los
  glifos/íconos que usan los prompts y la TUI de Pi.
...
Con los archivos `*-theme.auto.conf` presentes, kitty ≥ 0.38 alterna solo cuando macOS cambia entre modo claro y oscuro
— no hace falta ningún script.

## Actualizar esta copia

Si cambiás algo en `~/.config/kitty/kitty.conf`, regenerá la versión activa y verificá que coincida:

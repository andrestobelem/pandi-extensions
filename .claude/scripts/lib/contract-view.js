...
if(D.contract){var cEl=document.getElementById("contract");if(cEl){var safeMd=escapeMarkdownHtml(contractMd(D.contract));var html=(window.marked&&marked.parse)?marked.parse(safeMd,{breaks:false}):("<pre>"+esc(JSON.stringify(D.contract,null,2))+"</pre>");cEl.innerHTML=sanitizeRenderedHtml(html);}}

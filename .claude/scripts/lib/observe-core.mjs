...
import { createRequire as __pandiCreateRequire } from "node:module"; const require = __pandiCreateRequire(import.meta.url);
...
var require_cjs = __commonJS({
  "node_modules/deepmerge/dist/cjs.js"(exports, module) {
    ...
    function isSpecial(value) {
      var stringValue2 = Object.prototype.toString.call(value);
      return stringValue2 === "[object RegExp]" || stringValue2 === "[object Date]" || isReactElement(value);
    }
    ...
    !(function(t, e) {
      ...
      var _2 = (function() {
        ...
        return m3.parse = function(t2) {
          ...
          var $3, y3 = this, M4 = b2.p(d3), m4 = O2(r2), v3 = (m4.utcOffset() - this.utcOffset()) * e, g3 = this - m4, D3 = function() {
            return b2.m(y3, m4);
          };
          switch (M4) {
            case h:
        ...
        onopentag: function(name, attribs) {
          ...
          if (options.selfClosing.indexOf(name) !== -1) {
            ...
            if (frame.innerText && !hasText && !options.textFilter) {
              result += escapeHtml2(frame.innerText);
              addedText = true;
            }
          }
          if (skip) {
...
// extensions/pandi-dynamic-workflows/lib/agent-row-meta.ts
function listCount(value) {
  if (value === void 0) return void 0;
  if (Array.isArray(value)) return value.length > 0 ? value.length : void 0;
  const count = value.split(",").map((item) => item.trim()).filter(Boolean).length;
  return count > 0 ? count : void 0;
}
function shortAgentModel(model) {
  return model.split("/").filter(Boolean).pop() ?? model;
}
function buildAgentRowMetaChips(agent) {
  ...
  return [
    ...
    agent.schemaOk !== void 0 ? `schema:${agent.schemaOk ? "ok" : "bad"}` : "",
...
function agentRowMetaChipTone(label) {
...
// extensions/pandi-dynamic-workflows/lib/presentation.ts
...
// extensions/pandi-dynamic-workflows/observe/event-parser.ts
function formatAgentPhase(agent) {
  if (!agent.phaseIndex || !agent.phaseTotal) return void 0;
  const batch = agent.phaseId ? `P${agent.phaseId} ` : "";
  return `${batch}${agent.phaseIndex}/${agent.phaseTotal}`;
}

// extensions/pandi-dynamic-workflows/observe/safe-html.ts
function escapeHtml(value) {
  return value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;");
}
function safeRelativeHref(candidate) {
  if (!candidate) return void 0;
  if (/^[a-z][a-z0-9+.-]*:/i.test(candidate)) return void 0;
  if (candidate.startsWith("/") || candidate.startsWith("\\") || candidate.includes("\\")) return void 0;
...
function artifactViewerAnchor(candidate) {
  const safe = safeRelativeHref(candidate);
  if (!safe || !candidate) return void 0;
  return `artifact-${stableId(candidate)}-${safe.replaceAll("/", "-")}`;
}
function artifactViewerHref(candidate) {
  const anchor = artifactViewerAnchor(candidate);
  return anchor ? `artifact-viewer.html#${anchor}` : void 0;
}
function compactInlineForHtml(value, max = 220) {
  const oneLine = value.replace(/\bhttps?:\/\/[^\s<>"]+/gi, "[external-url]").replace(/\bjavascript:[^\s<>"]+/gi, "[unsafe-url]").replace(/\s+/g, " ").trim();
  if (oneLine.length <= max) return oneLine;
  return `${oneLine.slice(0, Math.max(0, max - 1))}\u2026`;
}

// extensions/pandi-dynamic-workflows/observe/html-agents.ts
function pillClass(state, ok) {
...
function meter(fraction, tone = "ok") {
...
function metricCard(label, value, detail = "") {
  return `<div class="metric-card"><div class="metric-label">${escapeHtml(label)}</div><div class="metric-value">${escapeHtml(String(value))}</div>` + (detail ? `<div class="metric-detail">${detail}</div>` : "") + `</div>`;
}
function agentFailed(agent) {
...
function plannedAgentTotal(agents) {
  const phaseTotals = /* @__PURE__ */ new Map();
  let standalone = 0;
  for (const agent of agents) {
    if (agent.phaseTotal !== void 0 && agent.phaseTotal > 0) {
      const key = agent.phaseId !== void 0 ? `phase:${agent.phaseId}` : `agent:${agent.name}`;
...
function summarizeProgress(model) {
  ...
  const tone = failed > 0 || model.state === "failed" ? "fail" : model.state === "running" || running > 0 || openEnded ? "run" : unknown > 0 || model.state === "cancelled" || model.state === "stale" || model.state === "unknown" || model.state === "planned" ? "warn" : "ok";
  return { observed, total, done, running, failed, unknown, fraction, tone, openEnded };
}
function progressValue(summary) {
  return `${summary.done}/${summary.total}${summary.openEnded ? "+" : ""}`;
}
function reportAgentPhaseDetail(agent) {
...
function agentAccessMeta(agent) {
  return [
    ...
    agent.outputTruncated ? "output truncated" : "",
    agent.outputChars !== void 0 ? `output chars: ${agent.outputChars}` : "",
...
function agentRowMeta(agent) {
  return buildAgentRowMetaChips(agent);
}
function miniChipClass(label) {
  return agentRowMetaChipTone(label);
}
function renderMiniChips(chips) {
  return `<div class="agent-chipline">${chips.map(
    (label) => `<span class="mini-chip${miniChipClass(label) ? ` ${miniChipClass(label)}` : ""}">${escapeHtml(label)}</span>`
  ).join("")}</div>`;
}
function renderMonitorAgentLine(agent) {
  const phase = formatAgentPhase(agent);
  const elapsed = agent.elapsedMs === void 0 ? "elapsed:\u2026" : `elapsed:${formatElapsedMs(agent.elapsedMs)}`;
  return `<div class="monitor-agent-row"><span class="monitor-agent-state ${pillClass(agent.state, agent.ok)}">${escapeHtml(agentStateText(agent))}</span><span class="mono">#${escapeHtml(String(agent.id))}</span>` + (phase ? `<span class="mono">${escapeHtml(phase)}</span>` : "") + `<b>${escapeHtml(agent.name)}</b><span class="kv muted">${escapeHtml(elapsed)}</span>` + (agent.code === void 0 ? "" : `<span class="kv muted">code:${escapeHtml(String(agent.code))}</span>`) + renderMiniChips(agentRowMeta(agent)) + `</div>`;
}
...
function renderMonitorSelectedAgent(agent, failed) {
  ...
  const outputState = [
    ...
    agent.outputChars !== void 0 ? `${agent.outputChars} chars` : ""
  ].filter(Boolean).join(" \u2022 ");
  const io = [
    agent.promptPreview ? detailLine("prompt preview", escapeHtml(compactInlineForHtml(agent.promptPreview, 220))) : "",
    outputState ? detailLine("output state", escapeHtml(outputState)) : "",
    agent.output !== void 0 ? detailLine("output", escapeHtml(compactInlineForHtml(agent.output.text, 220))) : "",
    agent.outputEmpty ? detailLine("integrity", "empty-output") : "",
    agent.outputTruncated ? detailLine(
      "integrity",
      `output:truncated${agent.outputChars === void 0 ? "" : ` (${escapeHtml(String(agent.outputChars))} chars)`}`
    ) : "",
    agent.stdoutTruncated ? detailLine(
      "integrity",
      `stdout:truncated${agent.stdoutChars === void 0 ? "" : ` (${escapeHtml(String(agent.stdoutChars))} chars)`}`
    ) : ""
  ].filter(Boolean).join("");
  return `<div class="callout ${failed ? "error" : "info"} monitor-selected"><b>Selected agent</b>` + detailLine(
    "agent",
    `#${escapeHtml(String(agent.id))} ${phaseToken ? `${escapeHtml(phaseToken)} ` : ""}${escapeHtml(agent.name)}`
  ) + detailLine(
    "state",
    `${escapeHtml(agent.state)}${agent.elapsedMs !== void 0 ? ` <span class="muted">\u2022</span> ${escapeHtml(formatElapsedMs(agent.elapsedMs))}` : ""}${agent.code !== void 0 ? ` <span class="muted">\u2022</span> code ${escapeHtml(String(agent.code))}` : ""}`
  ) + phase + detailLine(
    "prompt",
    `${escapeHtml(promptStatus)}${artifact ? ` <span class="muted">\u2022</span> ${artifact}` : ""}`
  ) + `<div class="monitor-subtitle">config</div>${config}` + (io ? `<div class="monitor-subtitle">i/o</div>${io}` : "") + `</div>`;
}
function renderWorkflowMonitor(model, summary) {
  const running = summary.running;
  const frac = summary.fraction;
  const last = model.logs.slice(-1)[0];
  const featured = model.agents.find(agentFailed) ?? model.agents.find((agent) => agent.state === "running") ?? model.agents[0];
  const row = (agent) => {
  ...
  const parallel = model.agentConcurrency !== void 0 ? `${running}/${model.agentConcurrency}` : String(running);
  const agentHeader = `<div class="monitor-agent-head"><b>Agents (${model.agents.length})</b><span>\u2022 parallel ${escapeHtml(parallel)}${model.peakParallelAgents === void 0 ? "" : ` \u2022 peak ${escapeHtml(String(model.peakParallelAgents))}`}</span></div>`;
  const featuredHint = featured ? renderMonitorSelectedAgent(featured, summary.failed > 0) : `<div class="callout info monitor-selected"><b>Selected agent</b><div class="monitor-detail-line muted">No agents recorded yet.</div></div>`;
  return `<section class="monitor-panel"><div class="monitor-head"><h2>Workflow monitor</h2><span class="rpill ${pillClass(model.state)}">${escapeHtml(model.state)}</span></div><div class="monitor-grid">` + metricCard(
    "Progress",
    progressValue(summary),
    `${meter(frac, summary.tone)} <span>${Math.round(frac * 100)}%</span>`
  ) + metricCard(
    "parallel",
    model.agentConcurrency !== void 0 ? `${running}/${model.agentConcurrency}` : running,
    model.peakParallelAgents !== void 0 ? `peak ${escapeHtml(String(model.peakParallelAgents))}` : "running now"
  ) + metricCard("failed", summary.failed, summary.failed ? "review failed cards" : "no failed agents") + metricCard(
    "artifacts",
    model.artifacts.length,
    model.artifacts[0] ? escapeHtml(model.artifacts[0].path) : "none listed"
  ) + metricCard("logs", model.logs.length, "timeline entries") + metricCard(
    "Latest activity",
    last ? `${String(last.time).slice(11, 19)} ${last.message}` : "\u2014",
    "last log event"
  ) + `</div>` + featuredHint + `<h2>Agent monitor</h2>` + agentHeader + (agentRows ? `<table class="monitor-table"><thead><tr><th>Monitor row</th></tr></thead><tbody>${agentRows}</tbody></table>` : `<div class="muted">No agents recorded for this run.</div>`) + `</section>`;
}
...
// extensions/pandi-dynamic-workflows/observe/markdown.ts
...
function safeMarkdownHref(raw) {
  ...
  if (href.startsWith("#")) {
    return /^#[A-Za-z0-9_-]+$/.test(href) ? href : void 0;
  }
...
function renderRunReportMarkdown(source) {
...
// extensions/pandi-dynamic-workflows/observe/html-builders.ts
function chip(label, value) {
  if (value === void 0 || value === "") return "";
  return `<span class="chip">${escapeHtml(label)}: ${escapeHtml(String(value))}</span>`;
}
function truncNote(t) {
...
function prettyJsonOutput(text) {
...
function markdownTableCell(value) {
  return markdownScalar(value).replace(/\|/g, "\\|").replace(/\r?\n+/g, " / ");
}
function flatRecordKeys(rows) {
  const keys = [];
...
function recordsToMarkdownTable(rows) {
...
function objectToKeyValueTable(record) {
...
function structuredValueMarkdown(value, level = 3) {
  if (Array.isArray(value)) {
    if (value.length === 0) return "_none_";
    if (value.every((item) => typeof item === "string")) return value.map((item) => `- ${item}`).join("\n");
    if (value.every(isJsonRecord)) return recordsToMarkdownTable(value);
    return value.map((item) => `- ${markdownScalar(item)}`).join("\n");
  }
  if (isJsonRecord(value)) {
    const scalarTable = objectToKeyValueTable(value);
    const nested = Object.entries(value).filter(([, nestedValue]) => !isJsonScalar(nestedValue)).map(
      ([key, nestedValue]) => `${"#".repeat(Math.min(level, 6))} ${humanizeKey(key)}
...
function structuredJsonMarkdown(value) {
  if (!isJsonRecord(value)) return structuredValueMarkdown(value);
  return Object.entries(value).map(([key, child]) => `### ${humanizeKey(key)}

${structuredValueMarkdown(child, 4)}`).join("\n\n");
}
function renderStructuredJson(text) {
  const parsed = parsedJsonOutput(text);
  if (!parsed) return void 0;
  return `<div class="structured-output"><div class="md-body">${renderRunReportMarkdown(structuredJsonMarkdown(parsed.value))}</div></div><details class="raw-json"><summary>Raw JSON</summary><div class="body"><pre class="json-output">${escapeHtml(parsed.pretty)}</pre></div></details>`;
}
function renderTextBody(text, render) {
  ...
  if (json !== void 0) return `<pre class="json-output">${escapeHtml(json)}</pre>`;
  return render === "markdown" ? `<div class="md-body">${renderRunReportMarkdown(text)}</div>` : `<pre>${escapeHtml(text)}</pre>`;
}
function textBlock(title, t, open = false, render = "pre") {
  if (!t) return "";
  const body = renderTextBody(t.text, render);
  return `<details${open ? " open" : ""}><summary>${escapeHtml(title)}${truncNote(t)}</summary><div class="body">${body}</div></details>`;
}

// extensions/pandi-dynamic-workflows/observe/html-css.ts
var PANDI_TOKENS_CSS = `:root {
  --bg: #242526;
  --paper: #292A2B;
  --info-bg: #2E2A33;
  --raised: #31353A;
...
/* mermaid.run({securityLevel:"sandbox"}) reemplaza el contenido de .mermaid por un <iframe>:
   el elemento en s\xED vive en la p\xE1gina padre y hereda este CSS (su documento interno no). */
.mermaid { margin: 8px 0 16px; }
.mermaid iframe { border:0; background:transparent; width:100%; display:block; }
`;

// extensions/pandi-dynamic-workflows/observe/html-mermaid.ts
function mermaidLabel(value) {
  return value.replace(/["<>{}[\]()|]/g, " ").replace(/\s+/g, " ").trim().slice(0, 90) || "step";
}
var MERMAID_CDN_VERSION = "11.15.0";
var MERMAID_CDN_URL = `https://cdn.jsdelivr.net/npm/mermaid@${MERMAID_CDN_VERSION}/dist/mermaid.min.js`;
var MERMAID_CDN_INTEGRITY = "sha384-yQ4mmBBT+vhTAwjFH0toJXNYJ6O4usWnt6EPIdWwrRvx2V/n5lXuDZQwQFeSFydF";
function mermaidStateClass(agent) {
  if (agent.ok === false || agent.state === "failed") return "failed";
  if (agent.state === "completed" || agent.ok === true) return "completed";
  if (agent.state === "running") return "running";
  return "other";
}
var MERMAID_STATE_STYLES = {
...
var MAX_DETAILED_MERMAID_AGENTS_PER_GROUP = 12;
function timestampMs(value) {
  if (!value) return void 0;
  const ms = Date.parse(value);
  return Number.isFinite(ms) ? ms : void 0;
}
function inferPhaseForAgent(agent, phases) {
  if (agent.phaseId !== void 0 || agent.phaseLabel) {
    const key2 = agent.phaseId !== void 0 ? `p${agent.phaseId}` : `label:${agent.phaseLabel}`;
    return { key: key2, label: agent.phaseLabel ?? `Phase ${agent.phaseId}` };
  }
  ...
  for (const phase of phases) {
    ...
    if (phaseMs === void 0 || phaseMs > ref || phaseMs < chosenMs) continue;
    chosen = phase;
    chosenMs = phaseMs;
  }
...
function modelPhaseKey(phase) {
  return `${phase.time}:${phase.label}`;
}
function buildRunMermaidGroups(model) {
...
function buildRunMermaidWaves(agents) {
  ...
  if (timed.some(({ started, ended }) => started === void 0 || ended === void 0 || ended < started))
    return [{ agents }];
  timed.sort((a, b2) => a.started - b2.started || a.ended - b2.ended || a.agent.id - b2.agent.id);
  const waves = [];
  let wave = [];
...
function agentStateCounts(agents) {
...
function summaryStateClass(counts) {
  if (counts.failed > 0) return "failed";
  if (counts.running > 0) return "running";
  if (counts.completed > 0 && counts.other === 0) return "completed";
  return "other";
}
function mermaidGroupSummaryLabel(agents) {
...
function buildRunMermaidSource(model) {
...
// extensions/pandi-dynamic-workflows/observe/html-report-sections.ts
function renderCallouts(model, autoRefreshSeconds) {
  const callouts = [];
  ...
  if (model.state === "running") {
    callouts.push(
      `<div class="callout info"><b>Point-in-time snapshot</b> as of ${escapeHtml(model.generatedAt)}` + (model.liveness === "unverified" ? ` \u2014 running as of ${escapeHtml(model.updatedAt ?? model.generatedAt)}; liveness unverified (out-of-session run dir).` : ". Outputs and metrics appear when the run completes.") + `</div>`
    );
  }
...
function renderSchemasSection(schemas) {
  ...
  return `<h2>Schemas (${schemas.length})</h2>` + schemas.map(
    (schema) => `<details><summary>${escapeHtml(schema.name)}</summary><div class="body"><pre class="json-output">${escapeHtml(schema.json)}</pre></div></details>`
  ).join("\n");
}
function renderHeaderChips(model, summary) {
  ...
  return [
    ...
    chip("elapsed", model.elapsedMs !== void 0 ? `${Math.round(model.elapsedMs / 1e3)}s` : void 0),
    chip("generated", model.generatedAt)
  ].join("");
}
function renderPhaseSection(model) {
  const phaseRows = model.phases.map(
    (p) => `<tr><td class="mono">${escapeHtml(p.time)}</td><td>${escapeHtml(p.label)}</td><td>${escapeHtml(p.source ?? "log")}</td></tr>`
  ).join("");
  if (!model.phases.length) return "";
  const hasStructuredPhases = model.phases.some((p) => p.source === "event");
  return `<h2>Phases</h2><div class="kv muted">${hasStructuredPhases ? "Structured phase events from the run dir; legacy log-derived phases are marked as log." : 'Derived from the "phase: \u2026" log convention.'}</div><table><thead><tr><th>Time</th><th>Phase</th><th>Source</th></tr></thead><tbody>${phaseRows}</tbody></table>`;
}
function renderLogSection(logs) {
  return logs.length ? `<details><summary>Timeline (${logs.length} log entries)</summary><div class="body">${renderTimeline(logs)}</div></details>` : "";
}
function renderTimelineDetails(details) {
  if (!details) return "";
  const pretty = prettyJsonOutput(details);
  const body = pretty ? `<pre class="json-output">${escapeHtml(pretty)}</pre>` : `<div class="kv muted">${escapeHtml(details)}</div>`;
  return `<div class="timeline-details">${body}</div>`;
}
function renderTimeline(logs) {
  const items = logs.map(
    (log) => `<li class="timeline-item"><span class="timeline-time">${escapeHtml(log.time)}</span><div class="timeline-message">${escapeHtml(log.message)}</div>${renderTimelineDetails(log.details)}</li>`
  ).join("");
  return `<ol class="timeline-list">${items}</ol>`;
}
function renderIntegritySection(integrity) {
  if (!integrity) return "";
  return `<h2>Result integrity</h2><div class="chips">` + [
...
function renderMetricsSection(t) {
  if (!t) return "";
  return `<h2>Run metrics</h2><div class="chips">` + [
...
function renderBasedOnSection(basedOn) {
  const basedOnRows = (basedOn ?? []).map((item) => {
    const detail = [item.role, item.desc].filter(Boolean).join(" \xB7 ");
    return `<tr><td>${escapeHtml(item.name)}</td><td>${escapeHtml(detail)}</td></tr>`;
  }).join("");
  return basedOnRows ? `<h2>Based on</h2><table><thead><tr><th>Scaffold/source</th><th>Role</th></tr></thead><tbody>${basedOnRows}</tbody></table>` : "";
}
function renderArtifactSection(artifacts, artifactsOmitted) {
  ...
  const artifactRows = artifacts.map((a) => {
    ...
    return `<tr><td>${cell}</td><td class="mono">${a.bytes !== void 0 ? escapeHtml(String(a.bytes)) : ""}</td></tr>`;
  }).join("");
  return `<h2>Artifacts</h2><table><thead><tr><th>File</th><th>Bytes</th></tr></thead><tbody>${artifactRows}</tbody></table>` + (artifactsOmitted ? `<div class="callout warn"><b>Clamp:</b> ${artifactsOmitted} more files not listed.</div>` : "");
}
function renderMermaidSection(model) {
  if (!model.agents.length) return "";
  return `<h2 id="run-diagram">Run diagram</h2><div class="mermaid">${escapeHtml(buildRunMermaidSource(model))}</div><script src="${MERMAID_CDN_URL}" integrity="${MERMAID_CDN_INTEGRITY}" crossorigin="anonymous"></script><script>mermaid.initialize({startOnLoad:false,securityLevel:"sandbox",theme:"base",themeVariables:window.matchMedia&&window.matchMedia("(prefers-color-scheme: light)").matches?{"background":"#F2F1F1","primaryColor":"#FFFFFF","primaryTextColor":"#222223","primaryBorderColor":"#C9C9C9","lineColor":"#0091FF66","clusterBkg":"#EDE4F8","clusterBorder":"#0091FF55","titleColor":"#676B79","fontFamily":"-apple-system, BlinkMacSystemFont, sans-serif"}:{"background":"#292A2B","primaryColor":"#31353A","primaryTextColor":"#E6E6E6","primaryBorderColor":"#3E4250","lineColor":"#6FC1FF66","clusterBkg":"#31353A","clusterBorder":"#6FC1FF55","titleColor":"#BBBBBB","fontFamily":"-apple-system, BlinkMacSystemFont, sans-serif"}});mermaid.run({querySelector:".mermaid"});</script><details><summary>Run diagram (Mermaid source text)</summary><div class="body"><div class="kv muted">Si el diagrama de arriba no renderiza (JS deshabilitado o CDN bloqueada), peg\xE1 este texto en un visor Mermaid (mermaid.live u otro).</div><pre>${escapeHtml(buildRunMermaidSource(model))}</pre></div></details>`;
}

// extensions/pandi-dynamic-workflows/observe/html-sections.ts
function openingText(model, summary) {
  ...
  if (summary.failed > 0) {
    return `${summary.failed} de ${totalAgents} ${plural(totalAgents, "agente")} fall\xF3${summary.failed === 1 ? "" : "n"}. Las tarjetas fallidas est\xE1n abiertas abajo; empez\xE1 por ellas y luego revis\xE1 el output final si existe.`;
  }
  if (model.state === "running") {
    return `Instant\xE1nea del run: ${totalAgents} ${agentLabel} ${recordedLabel} hasta ahora. El run sigue en progreso, as\xED que outputs y m\xE9tricas pueden cambiar.`;
  }
  if (model.state === "cancelled") {
    return `El run fue cancelado con ${totalAgents} ${agentLabel} registrados. Us\xE1 la timeline y las tarjetas de agentes para encontrar el \xFAltimo paso confiable.`;
  }
  if (model.state === "stale") {
    return `El run parece stale: hay ${totalAgents} ${agentLabel} ${recordedLabel}, pero esta sesi\xF3n no confirma un owner activo. Trat\xE1 este reporte como diagn\xF3stico, no como veredicto final.`;
  }
  if (model.state === "completed") {
    return `${totalAgents} ${agentLabel} completaron el run sin fallas registradas. Empez\xE1 por el output final si existe; los detalles crudos quedan debajo para depurar.`;
  }
  if (model.state === "planned") {
    return `Vista previa pre-launch: ${totalAgents} ${agentLabel} planificados; nada corri\xF3 todav\xEDa. Revis\xE1 prompts, schemas y el script completo abajo \u2014 al terminar el run, el reporte real reemplaza esta vista.`;
  }
  return `Estado del run: ${model.state}. Hay ${totalAgents} ${agentLabel} registrados; revis\xE1 primero los callouts y despu\xE9s las tarjetas de agentes.`;
}
function renderAgent(agent) {
  ...
  if (agent.elapsedMs !== void 0) meta.push(`elapsed ${Math.round(agent.elapsedMs / 100) / 10}s`);
  if (agent.code !== void 0) meta.push(`code ${agent.code}`);
  if (agent.killed) meta.push("killed");
  if (agent.schemaOk !== void 0) meta.push(`schema ${agent.schemaOk ? "ok" : "FAILED"}`);
  if (agent.outputEmpty) meta.push("output:empty");
  if (agent.outputTruncated) meta.push("output:truncated");
  if (agent.outputChars !== void 0) meta.push(`output chars ${agent.outputChars}`);
  if (agent.phaseLabel) meta.push(`phase ${agent.phaseLabel}`);
  const m2 = agent.metrics;
  if (m2?.costTotal !== void 0) meta.push(`cost ${m2.costTotal}`);
  if (m2?.totalTokens !== void 0) meta.push(`tokens ${m2.totalTokens}`);
  if (m2?.toolCalls !== void 0) meta.push(`tools ${m2.toolCalls}${m2.toolErrors ? ` (${m2.toolErrors} err)` : ""}`);
  ...
  if (agent.inlineOmitted) {
    ...
    if (agent.output !== void 0) body += textBlock("Output", agent.output, false, "structured");
  ...
  if (agent.outputEmpty || agent.outputTruncated || agent.stdoutTruncated) {
    const facts = [
      agent.outputEmpty ? "empty-output" : "",
      agent.outputTruncated ? `output:truncated${agent.outputChars === void 0 ? "" : ` (${agent.outputChars} chars)`}` : "",
      agent.stdoutTruncated ? `stdout:truncated${agent.stdoutChars === void 0 ? "" : ` (${agent.stdoutChars} chars)`}` : ""
    ].filter(Boolean);
    body += `<div class="callout warn"><b>Result integrity:</b> ${escapeHtml(facts.join(" \xB7 "))}. Full stdout remains linked when available.</div>`;
  }
  if (agent.stderrTail) {
  ...
  return `<details class="${failed ? "fail-card" : ""}"${failed ? " open" : ""}><summary>${pill} <b>#${agent.id} ${escapeHtml(agent.name)}</b> <span class="kv muted">${escapeHtml(meta.join(" \xB7 "))}</span>` + (links.length ? ` <span class="kv">${links.join(" \xB7 ")}</span>` : "") + `</summary><div class="body">${body}</div></details>`;
}

// extensions/pandi-dynamic-workflows/observe/html.ts
function buildRunReportHtml(model) {
  const summary = summarizeProgress(model);
  const statePill = `<span class="rpill ${pillClass(model.state)}">${escapeHtml(model.state)}</span>`;
  const autoRefreshSeconds = model.state === "running" && model.autoRefreshSeconds !== void 0 ? Math.max(1, Math.round(model.autoRefreshSeconds)) : void 0;
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
${autoRefreshSeconds !== void 0 ? `<meta http-equiv="refresh" content="${autoRefreshSeconds}">` : ""}
...
<div class="container">
<header>
<div class="kicker">Pandi artifact \xB7 workflow run report</div>

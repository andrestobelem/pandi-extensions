...
document.getElementById("agents").innerHTML=D.nodes.map((n,i)=>'<div class="card"><div class="head"><span class="caret">▸</span><span class="badge" style="background:'+(pc[n.phase]||"#8a877f")+'">'+esc(n.phase)+'</span><span class="nid">'+esc(n.id)+'</span><span class="me" title="model · effort">'+esc(meLabel(n))+'</span>'+rpill(n)+'<span class="schema">schema: '+esc(n.schema)+'</span></div><div class="body"><div class="meta-row"><span><b>phase</b> '+esc(n.phase)+'</span><span><b>model</b> '+esc(n.model)+'</span><span><b>effort</b> '+esc(n.effort)+'</span><span><b>tools</b> '+esc(n.tools)+'</span>'+(n.runtimeOnly?'<span><b>source</b> runtime-only (declared phase/model inferred)</span>':'')+'</div>'+(n.run&&n.run.output?'<div class="rout"><span class="lbl">runtime output · '+esc(n.id)+(n.run.count>1?' (first of '+n.run.count+')':'')+'</span>'+esc(n.run.output)+'</div>':'')+(n.run&&n.run.artifact?'<div class="rart">artifact: '+esc(n.run.artifact)+'</div>':'')+'<div class="prompt"><button class="copy" data-c="'+i+'">copy</button>'+esc(n.prompt.trim())+'</div></div></div>').join("");

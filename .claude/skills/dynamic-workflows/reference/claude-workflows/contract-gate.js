...
if (planResources && routing && routing.shape === "dynamic-workflow" && routing.pattern) {
	...
	const planned = await agent(
		`Recomendá un budget por nodo de model + reasoning-effort para CORRER el workflow "${routing.pattern}" en la tarea descrita por el contrato de abajo.\n` +
			`Primero leé ~/.claude/workflows/${routing.pattern}.js (o el del proyecto ./.claude/workflows/${routing.pattern}.js) y extraé sus role keys node('<role>', …); emití UNA entrada de plan por role. Si ese archivo no existe, inferí nombres de role razonables desde el patrón y decilo en rationale.\n` +

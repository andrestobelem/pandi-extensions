...
const catalog = await agent(
	`Listá los EXISTING Claude Code dynamic workflows disponibles para reuse/compose. Leé el catálogo de usuario en ~/.claude/workflows/*.js y, si existe, el catálogo del proyecto en .claude/workflows/*.js. Para CADA archivo — EXCLUÍ "workflow-factory" mismo y cualquier cosa bajo un subdirectorio drafts/ — extraé meta.name y meta.description, y clasificá kind como "lib" (un sub-workflow reusable, p. ej. un nombre que termina en -lib), "composed" (llama workflow(...) / está construido desde otros), o "base". Devolvé { workflows: [ { name, description, kind } ] }.`,

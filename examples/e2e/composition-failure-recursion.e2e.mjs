/**
 * Durable behavioral e2e for the FAILURE + RECURSION contracts of ctx.workflow()
 * composition in extensions/dynamic-workflows.ts.
 *
 * The sibling suite (dynamic-workflow-composition.e2e.mjs) pins the HAPPY-PATH
 ...
 *      that prevents an infinite run, and it had zero e2e coverage.
 *
 ...
 *
 * Same self-bootstrapping pattern as the other e2es: esbuild the CURRENT source to a
 ...
 *
 * Run it:
 *   node examples/e2e/composition-failure-recursion.e2e.mjs
 */
...
async function buildExtension() {
	const outDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-dwf-fail-e2e-"));

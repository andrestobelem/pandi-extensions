/**
 * lib/rank-candidates — reusable composable sub-workflow (the lib/ convention).
 *
 * A second composable building block alongside lib/verify-claims, demonstrating
 * that ctx.workflow() composition is a general pattern, not a one-off. Where
 * lib/verify-claims FILTERS a set (verified vs dropped), this one ORDERS a set:
 * it scores candidates against a rubric with an independent jury and returns a
 * ranking. Pairs naturally with the catalog's `tournament` use-case
 * ("Rank candidate designs pairwise", "Choose the best generated prompt").
 *
 * Contract:
 *   input:  { candidates: [{ id?, text }], rubric?: string, goal?: string,
 *             jurors?: number, keepTop?: number }
 *   output: { ranked: [{ id, text, score, votes, rationale }],   // best-first
 *             best: <ranked[0] | null>,
 *             dropped: [{ id, text, reason }],                    // unscorable
 *             coverage: { candidates, jurors, requestedJurors } }
 *
 * RESOLUTION / HOW TO RUN THIS (read before running):
 *   ctx.workflow(name) resolves `name` from the runtime's WORKFLOW DIRECTORY
 *   (project .pi/workflows or global ~/.pi/agent/workflows), NOT from examples/.
 *   So to actually run the composition, copy BOTH files into a workflow dir,
 *   preserving the lib/ path:
 *
 ...
 *     pi   # then: /dynamic-workflow run composition-rank-driver '{"goal":"best CLI name"}'
 *
 *   The driver calls ctx.workflow("lib/rank-candidates", ...). Because the path
 *   under .pi/workflows matches the string passed to ctx.workflow(), it resolves;
 *   if you flatten the lib/ directory it will NOT resolve. The files here under
 *   examples/ are reference copies — see examples/e2e/composition-rank.e2e.mjs
 *   for a runnable proof that this layout is resolvable and coherent.
 */

 ...
 * it, then write `.pi/workflows/generated/<slug>.js` by default.
 *
 * Input: { task: "...", name?: "generated/<slug>", write?: boolean }
...
module.exports = async function workflow(ctx, input) {
  ...
  const slug = (value) => String(value)
    ...
    .replace(/(^|\/)\.\.(?=\/|$)/g, "")
    .replace(/^-+|-+$/g, "")
    .slice(0, 80) || "generated-workflow";
  const safeName = slug(input?.name ?? `generated/${slug(task)}`);
  const workflowName = safeName.endsWith(".js") ? safeName.slice(0, -3) : safeName;
  const workflowPath = `.pi/workflows/${workflowName}.js`;
  ...
  const implement = await ctx.agent(
    `Generate a COMPLETE JavaScript Pi dynamic workflow for this task. Return ONLY JavaScript, no Markdown fences.\n\n` +
      ...
      `- Use read-only subagent tools unless the task explicitly requires mutation.\n` +
